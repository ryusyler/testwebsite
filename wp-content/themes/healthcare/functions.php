...
//Social Settings
		...
		'footer_copyright' =>__('Copyright © All Rights Reserved. 2019 | Theme Developed by','healthcare'),
...
function healthcare_activation_notice(){
	wp_enqueue_style('enigma-font-awesome', get_template_directory_uri() . '/css/font-awesome/css/font-awesome.min.css');
	...
	$isitdismissed = 'healthcare_notice_dismissed'.$currentversion;
	if ( !get_user_meta( get_current_user_id() , $isitdismissed ) ) { ?>
			<!---our-product-features--->	 
		<div class="notice notice-success">					
		<p class="notice-text">
		<?php $theme_info = wp_get_theme();
			  printf( esc_html__('Thank you for installing %1$s ¬ Version %2$s ,', 'healthcare'), esc_html( $theme_info->Name ), esc_html( $theme_info->Version ) );		
			  echo esc_html__( 'For More info  about Premium Products & offers, Do visit our welcome page.', 'healthcare' ); ?>
		</p>
		<p class="notic-gif"><a class="pro" target="_self" href="<?php echo admin_url('/themes.php?page=healthcare') ?>"><img src="<?php echo get_template_directory_uri(); ?>/images/wlcm.gif"></a></p>
		<a class="dismiss" href="?-notice-dismissed<?php echo $currentversion;?>"><i class="fa fa-times" ></i></strong></a>
		</div>	
<?php } } 
function healthcare_notice_dismissed() {
	$wl_th_info = wp_get_theme(); 
	$currentversion = str_replace('.','',(esc_html( $wl_th_info->get('Version') )));
...
add_action( 'admin_init', 'healthcare_notice_dismissed' );